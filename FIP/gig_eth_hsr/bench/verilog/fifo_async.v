...
module fifo_async #(parameter FDW  =17,  // fifo data width
...
(
   ...
   //---------------------------------------------------
   fifo_async_core #(.FDW(FDW), .FAW(FAW))
   u_fifo (
         .Data_out   (rd_dout      ),
         .Empty_out  (empty        ),
         .ReadEn_in  (rd_enable    ),
...
//----------------------------------------------------------------
module fifo_async_core #(parameter FDW = 8, // data width
                                       FAW = 4) // address width
(
       Reset_in
     , Clear_in
    ...
    fifo_async_core_gray_counter #(.CW(FAW)) GrayCounter_pWr (
    ...
    fifo_async_core_gray_counter #(.CW(FAW)) GrayCounter_pRd (
...
// Date     : 15/May/2005.
//=======================================
module fifo_async_core_gray_counter #(parameter CW = 4)
(      input  wire          Reset_in 
