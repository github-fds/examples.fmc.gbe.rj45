...
int parser_eth_packet_core(uint8_t *pkt, int leng, int level)
{
  ...
  if (leng>=(idx+2)) {
   ...
   if (--level>0) {
      ...
      case 0x8808: printf(" (Ethernet flow control)\n"); break;
      case 0x88CC: printf(" (LLDP  packet)\n"); break;
      case 0x88F7: return parser_ptp_packet_core(pkt+ETH_HDR_LEN, leng-ETH_HDR_LEN, level); break;
    //case 0x88F7: printf(" (PTPv2 raw packet)\n"); break;
      case 0x88FB: printf(" (PRP   packet)\n"); break;
...
int parser_ptp_packet_core(uint8_t *pkt, int leng, int level)
{
    ...
    printf("PTPv2 messageType        0x%01X ", hdr->messageType); //pkt[0]&0x0F);
    switch (hdr->messageType) {
    case 0x0: printf("(PTPV2_MSG_Sync)\n"); break;
    case 0x1: printf("(PTPV2_MSG_Delay_Req)\n"); break;
    case 0x2: printf("(PTPV2_MSG_Pdelay_Req)\n"); break;
    case 0x3: printf("(PTPV2_MSG_Pdelay_Resp)\n"); break;
    case 0x8: printf("(PTPV2_MSG_Follow_Up)\n"); break;
    case 0x9: printf("(PTPV2_MSG_Delay_Resp)\n"); break;
    case 0xA: printf("(PTPV2_MSG_Pdelay_Resp_Follow_Up)\n"); break;
    case 0xB: printf("(PTPV2_MSG_Announce)\n"); break;
    case 0xC: printf("(PTPV2_MSG_Signaling)\n"); break;
    case 0xD: printf("(PTPV2_MSG_Management)\n"); break;
    default:  printf("\n"); break;
    }
    printf("PTPv2 versionPTP         0x%01X\n", hdr->versionPTP); //pkt[1]&0x0F);
    printf("PTPv2 messageLength      0x%04X\n", ntohs(hdr->messageLength)); //(pkt[2]<<8)|pkt[3]);
    printf("PTPv2 domainNumber       0x%02X\n", hdr->domainNumber);
    printf("PTPv2 Flags              0x%04X\n", ntohs(hdr->Flags));
    printf("PTPv2 clockIdentity      0x%02X%02X%02X%02X%02X%02X%02X%02X\n"
