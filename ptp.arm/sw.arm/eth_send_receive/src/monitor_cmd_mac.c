...
//--------------------------------------------------------
// ptp_csr
int func_ptp_csr(int argc, char* argv[])
{
    ptp_csr_check();
    return 0;
}
...
//--------------------------------------------------------
//static uint8_t  ptp_flag=0;
//--------------------------------------------------------
// ptp_init [-a ptp_mac_addr]
//          [-b val]        //control_promiscuous=0;
//          [-c val]        //control_drop_non_ptp=1;
//          [-d val]        //control_enable_qr=1;
//          [-e val]        //control_snoop=0;
//          [-s]            //status
int func_ptp_init(int argc, char* argv[])
{
  //int flag_status=0;
    int flag_help=0;
    int pos=1;
...
//------------------------------------------------------------------------------
static void nops(unsigned int num) {
    volatile int i;
    for(i = 0; i < num; i++) {
        asm("nop");
    }
...
//          [-t vlan   ]
//          [-r]         // repeat
//          [-i num    ] // interval for repeat case
    ...
    while ((pos<argc)&&(argv[pos][0]=='-')) {
       switch (argv[pos][1]) {
          ...
          case 'r': flag_repeat = 1;
                    break;
          case 'i': flag_interval = 1;
                    if (argc<=pos) return -1;
                    interval = (int)strtoi(token);
                    break;
          case 'x': pos++;
                    if (argc<=pos) return -1;
...
int mac_register(void)
{
  ...
  command_register("ptp_csr" , "", func_ptp_csr,
                   "ptp_csr  : check ptp CSR",
                   "ptp_csr                                     : check ptp CSR");
  ...
  command_register("hsr_init", "", func_hsr_init,
                   "hsr_init : initialize HSR",
                   "hsr_init [-a hsr_mac_addr]                  : initialize HSR\n"
                   "         [-b val]        //control_promiscuous=0;\n"
                   "         [-c val]        //control_drop_non_hsr=1;\n"
                   "         [-d val]        //control_enable_qr=1;\n"
                   "         [-e val]        //control_snoop=0;\n"
                   "         [-s]            //status");
  command_register("ptp_init", "", func_ptp_init,
                   "ptp_init : initialize PTP",
                   "ptp_init [-a ptp_mac_addr]                  : initialize PTP\n"
                   "         [-b val]        //control_promiscuous=0;\n"
                   "         [-c val]        //control_drop_non_hsr=1;\n"
                   "         [-d val]        //control_enable_qr=1;\n"
                   "         [-e val]        //control_snoop=0;\n"
  ...
  command_register("pkt_snd", "", func_pkt_snd,
                   ...
                   "         [-r]                  //repeat\n"
                   "         [-i num]              //interval between packets for repeat\n"
...
// Revision History
//
// 2019.05.30: 'nop()' for 'pkt_snd()' added.
