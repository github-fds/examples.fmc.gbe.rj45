...
//------------------------------------------------------------------------------
   ...
   `endif
//------------------------------------------------------------------------------
   wire        ptp_gmii_tx_clk;
   wire [ 7:0] ptp_gmii_txd   ;
   wire        ptp_gmii_txen  ;
   wire        ptp_gmii_txer  ;
   wire        ptp_gmii_rx_clk;
   wire [ 7:0] ptp_gmii_rxd   ;
   wire        ptp_gmii_rxdv  ;
   wire        ptp_gmii_rxer  ;
//------------------------------------------------------------------------------
   gig_eth_mac_ptp_axi
   ...
   u_mac (
     ...
     `else
     , .gmii_gtxc       ( gmiiU_gtxc ) // be careful its direction (output)
     , .gmii_txd        ( gmiiU_txd  ) // be careful its direction (output)
     , .gmii_txen       ( gmiiU_txen ) // be careful its direction (output)
     , .gmii_txer       ( gmiiU_txer ) // be careful its direction (output)
     , .gmii_rxc        ( gmiiU_rxc  ) // be careful its direction (input)
     , .gmii_rxd        ( gmiiU_rxd  ) // be careful its direction (input)
     , .gmii_rxdv       ( gmiiU_rxdv ) // be careful its direction (input)
     , .gmii_rxer       ( gmiiU_rxer ) // be careful its direction (input)
     ...
     `endif
   //`ifdef IEEE1588_PTP
     , .ptp_gmii_tx_clk ( ptp_gmii_tx_clk )
     , .ptp_gmii_txd    ( ptp_gmii_txd    )
     , .ptp_gmii_txen   ( ptp_gmii_txen   )
     , .ptp_gmii_txer   ( ptp_gmii_txer   )
     , .ptp_gmii_rx_clk ( ptp_gmii_rx_clk )
     , .ptp_gmii_rxd    ( ptp_gmii_rxd    )
     , .ptp_gmii_rxdv   ( ptp_gmii_rxdv   )
     , .ptp_gmii_rxer   ( ptp_gmii_rxer   )
   //`endif
...
//------------------------------------------------------------------------------
    gig_eth_ptpv2_lite_apb
              `ifdef SIM
              #(.PTP_GMII_TX_SYNCHRONOUS(1))
              `endif
    u_ptpv2_lite  (
       .PRESETn     ( PRESETn     )
     , .PCLK        ( PCLK        ) // ACLK == gtx0_clk125mhz
     , .PSEL        ( PSEL     [4])
     , .PENABLE     ( PENABLE     )
     , .PADDR       ( PADDR       )
     , .PWRITE      ( PWRITE      )
     ...
     , .IRQ_PTP     ( irq_ptp     )
     , .IRQ_RTC     ( irq_rtc     )
     , .gmii_rx_clk ( ptp_gmii_rx_clk )
     , .gmii_rxd    ( ptp_gmii_rxd    )
     , .gmii_rxdv   ( ptp_gmii_rxdv   )
     , .gmii_rxer   ( ptp_gmii_rxer   )
     , .gmii_tx_clk ( ptp_gmii_tx_clk )
     , .gmii_txd    ( ptp_gmii_txd    )
     , .gmii_txen   ( ptp_gmii_txen   )
     , .gmii_txer   ( ptp_gmii_txer   )
     , .rtc_clk     ( gtx_clk         ) // == gtx0_clk125mhz
    ...
    `ifdef AMBA_APB3
    assign PREADY [4]=1'b1;
    assign PSLVERR[4]=1'b0;
    `endif
    assign ptp_ready=1'b1;
//------------------------------------------------------------------------------
// HSR not used
//------------------------------------------------------------------------------
   //---------------------------------------------------------------------------
