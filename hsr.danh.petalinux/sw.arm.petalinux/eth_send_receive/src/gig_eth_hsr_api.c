...
#else
#define CSRA_HSR_VERSION       (pHSR+0x00/4)
#define CSRA_HSR_MAC_ADDR0     (pHSR+0x10/4)// MAC[47:16]
#define CSRA_HSR_MAC_ADDR1     (pHSR+0x14/4)// MAC[15:0]
#define CSRA_HSR_HSR_NET_ID    (pHSR+0x18/4)
#define CSRA_HSR_CONTROL       (pHSR+0x1C/4)
#define CSRA_HSR_PHY           (pHSR+0x20/4)// to check and drive PHY RESET
#define CSRA_HSR_PROXY         (pHSR+0x24/4)// read-only (num of entries)
#define CSRA_HSR_QR            (pHSR+0x28/4)// read-only (num of entries)
#define CSRA_HSR_CRC_ERR_HOST  (pHSR+0x30/4)// read-only
#define CSRA_HSR_RCV_PKT       (pHSR+0x40/4)// read-only
#define CSRA_HSR_CRC_ERR       (pHSR+0x44/4)// read-only
#define CSRA_HSR_DROP_UNKNOWN  (pHSR+0x48/4)// read-only
#define CSRA_HSR_DROP_FULL     (pHSR+0x4C/4)// read-only
#define CSRA_HSR_DROP_SRC      (pHSR+0x50/4)// read-only
#define CSRA_HSR_DROP_NON_HSR  (pHSR+0x54/4)// read-only
#define CSRA_HSR_DROP_NON_QR   (pHSR+0x58/4)// read-only
#define CSRA_HSR_BOTH          (pHSR+0x5C/4)// read-only
#define CSRA_HSR_UPSTREAM      (pHSR+0x60/4)// read-only
#define CSRA_HSR_FORWARD       (pHSR+0x64/4)// read-only
#endif
