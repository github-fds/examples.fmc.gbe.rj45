...
#else
#define CSRA_GBE_CONTROL    (pMAC+0x00/4)
#define CSRA_GBE_STATUS     (pMAC+0x04/4)
#define CSRA_GBE_VERSION    (pMAC+0x08/4)

#define CSRA_GBE_MAC_ADDR0  (pMAC+0x10/4) //[7:0] goes to conf_mac_addr[47:40]
#define CSRA_GBE_MAC_ADDR1  (pMAC+0x14/4)

#define CSRA_GBE_CONF_TX0   (pMAC+0x20/4)
#define CSRA_GBE_CONF_TX1   (pMAC+0x24/4)
#define CSRA_GBE_CONF_RX0   (pMAC+0x30/4)
#define CSRA_GBE_CONF_RX1   (pMAC+0x34/4)

#define CSRA_GBE_DES_TX0    (pMAC+0x40/4) // room,bnum
#define CSRA_GBE_DES_TX1    (pMAC+0x44/4) // dummy to align with 64-bit
#define CSRA_GBE_DES_TX2    (pMAC+0x48/4) // src
#define CSRA_GBE_DES_TX3    (pMAC+0x4C/4) // src

#define CSRA_GBE_DES_RX0    (pMAC+0x50/4) // items,bnum
#define CSRA_GBE_DES_RX1    (pMAC+0x54/4) // dummy to align with 64-bit
#define CSRA_GBE_DES_RX2    (pMAC+0x58/4) // dst
#define CSRA_GBE_DES_RX3    (pMAC+0x5C/4) // dst

#define CSRA_GBE_DMA_TX0    (pMAC+0x60/4) // control (chunk)
#define CSRA_GBE_DMA_TX1    (pMAC+0x64/4) // status (full or empty)
#define CSRA_GBE_DMA_TX2    (pMAC+0x68/4) // start (lower 32-bit)
#define CSRA_GBE_DMA_TX3    (pMAC+0x6C/4) // start (upper 32-bit)
#define CSRA_GBE_DMA_TX4    (pMAC+0x70/4) // end
#define CSRA_GBE_DMA_TX5    (pMAC+0x74/4) // end
#define CSRA_GBE_DMA_TX6    (pMAC+0x78/4) // head
#define CSRA_GBE_DMA_TX7    (pMAC+0x7C/4) // head
#define CSRA_GBE_DMA_TX8    (pMAC+0x80/4) // tail (RO)
#define CSRA_GBE_DMA_TX9    (pMAC+0x84/4) // tail (RO)

#define CSRA_GBE_DMA_RX0    (pMAC+0x90/4) // control (chunk)
#define CSRA_GBE_DMA_RX1    (pMAC+0x94/4) // status (full or empty)
#define CSRA_GBE_DMA_RX2    (pMAC+0x98/4) // start
#define CSRA_GBE_DMA_RX3    (pMAC+0x9C/4) // start
#define CSRA_GBE_DMA_RX4    (pMAC+0xA0/4) // end
#define CSRA_GBE_DMA_RX5    (pMAC+0xA4/4) // end
#define CSRA_GBE_DMA_RX6    (pMAC+0xA8/4) // head (RO)
#define CSRA_GBE_DMA_RX7    (pMAC+0xAC/4) // head (RO)
#define CSRA_GBE_DMA_RX8    (pMAC+0xB0/4) // tail
#define CSRA_GBE_DMA_RX9    (pMAC+0xB4/4) // tail
#endif
...
printf("%s() tx REGWRB(0x%08X, loc, %d)\n", __FUNCTION__, (unsigned int)head, brem/4);
                ...
                #else
                uint32_t dst=(uint32_t)pTXBUFF+(head-ADDR_GBE_BRAM_TX_START);
                memcpy((void*)dst,(void*)loc,(brem/4)*4);
                #endif
                head += (brem/4)*4;
                brem -= (brem/4)*4;
...
printf("%s() tx REGWR(0x%08X, 0x%08X)\n", __FUNCTION__, (unsigned int)head, (unsigned int)value);
                ...
                #else
                uint32_t dst=(uint32_t)pTXBUFF+(head-ADDR_GBE_BRAM_TX_START);
                REGWR(dst, value); // it may cause page-fault due to non-word aligned packet
                #endif
                head += brem;
                brem  = 0;
                if (head>tx_end) return -1;
...
printf("%s() tx REGWRB(0x%08X, loc, %d)\n", __FUNCTION__, (unsigned int)head, rooms/4);
            ...
            #else
            uint32_t dst=(uint32_t)pTXBUFF+(head-ADDR_GBE_BRAM_TX_START);
            memcpy((void*)dst,(void*)loc,(rooms/4)*4);
            #endif
            head += rooms;
            loc  += rooms;
            brem -= rooms;
...
printf("%s() rx REGRDB(0x%08X, loc, %d) %d\n", __FUNCTION__, (unsigned int)tail, brem/4, items);
                ...
                #else
                uint32_t src=(uint32_t)pRXBUFF+(tail-ADDR_GBE_BRAM_RX_START);
                memcpy((void*)loc,(void*)src,(brem/4)*4);
                #endif
                tail += (brem/4)*4;
                brem -= (brem/4)*4;
...
printf("%s() brem should be less than 4, but %d\n", __FUNCTION__, brem);
            ...
            if (brem>0) {
                ...
                #else
                uint32_t src=(uint32_t)pRXBUFF+(tail-ADDR_GBE_BRAM_RX_START);
                REGRD(src, value); // it may cause page-fault due to non-word aligned packet
                #endif
#if defined(VERBOSE)
...
printf("%s() tx REGRDB(0x%08X, loc, %d)\n", __FUNCTION__, (unsigned int)tail, items/4);
            ...
            #else
            uint32_t src=(uint32_t)pRXBUFF+(tail-ADDR_GBE_BRAM_RX_START);
            memcpy((void*)loc,(void*)src,(items/4)*4);
            #endif
            tail += items;
            loc  += items;
            brem -= items;
